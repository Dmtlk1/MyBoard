...
def user_support_finl(user_name):
    keyboard = InlineKeyboardMarkup(
        ...
        InlineKeyboardButton("💌 Написать оператору", url=f"https://t.me/{user_name}"),
    )

    return keyboard

# Открыть оператора
def user_market_finl(user_name):
    keyboard = InlineKeyboardMarkup(
    ).add(
        InlineKeyboardButton("☎️ Открыть магазин", url=f"https://t.me/{user_name}"),
    )
