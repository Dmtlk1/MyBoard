...
# Принятие нового айди для магазина
@dp.message_handler(IsAdmin(), state="here_settings_market")
async def settings_market_get(message: Message, state: FSMContext):
    if message.text.isdigit():
        get_user = get_userx(user_id=message.text)
        ...
        if get_user is not None:
            if len(get_user['user_login']) >= 1:
                await state.finish()
                update_settingsx(misc_market=get_user['user_id'])
